 ...
 */
static microrlr_t
prv_cmdline_buf_split(microrl_t *mrl, const char **tkn_str_arr, uint8_t *tkn_cnt_ptr, size_t limit)
{
    uint8_t num = 0;
...
static microrlr_t prv_cmdline_buf_insert_text(microrl_t *mrl, const char *text_str, size_t len)
{
    ...
    mrl->cursor += len;
    mrl->cmdlen += len;
    memset(&mrl->cmdline_str[mrl->cmdlen],
           0x00,
           MICRORL_ARRAYSIZE(mrl->cmdline_str) - 1 - mrl->cmdlen);
...
#else
        str_ptr = prv_cursor_generate_move(
            str_ptr,
            -(MICRORL_ARRAYSIZE(mrl->cmdline_str) - 1 + mrl->prompt_size + 2));
        str_ptr = prv_cursor_generate_move(str_ptr, mrl->prompt_size + pos);
 ...
 */
static size_t
prv_hist_restore_line(microrl_hist_rbuf_t *rbuf_ptr, char *line_str, microrl_hist_dir_t dir)
{
    size_t cnt = 0;
    ...
    memset(line_str,
           0x00,
           MICRORL_CFG_CMDLINE_LEN); /* Placing the found record on the command line */
...
static void prv_hist_save_line(microrl_hist_rbuf_t *rbuf_ptr, char *line_str, size_t len)
{
    ...
    prv_hist_restore_line(rbuf_ptr,
                          last_record_str,
...
static microrlr_t prv_complete_get_input(microrl_t *mrl)
{
    ...
    if (len != 0) {
        prv_cmdline_buf_insert_text(mrl,
                                    cmplt_tkn_arr[0] + strlen(tkn_str_arr[tkn_cnt - 1]),
 ...
 *
 * Use \ref MICRORL_ECHO_ONCE to disable echo for password input, echo mode will enabled after user
 * press 'Enter'. Use \ref MICRORL_ECHO_ON or \ref MICRORL_ECHO_OFF to turn on or off the echo
 * manually.
 *
 * \param[in,out]   mrl: \ref microrl_t working instance
...
microrlr_t microrl_processing_input(microrl_t *mrl, const void *data_ptr, size_t len)
{
    ...
    while (len-- != 0) {
        ...
        if ((ch == MICRORL_ESC_ANSI_CR) || (ch == MICRORL_ESC_ANSI_LF)) {
             ...
             */
            if (mrl->last_endl ==
                (ch == MICRORL_ESC_ANSI_CR ? MICRORL_ESC_ANSI_LF : MICRORL_ESC_ANSI_CR)) {
        ...
        if (IS_CONTROL_CHAR(ch)) {
            ...
            if ((ch == ' ') &&
                (mrl->cmdlen == 0)) { /* Skip spaces before first command line symbol */
                break;
            }
