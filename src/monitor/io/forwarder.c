...
{
    uintptr_t base = PD_IO_MONITOR_SLOT_BASE + cid * PD_IO_MONITOR_SLOT_SIZE;

    /*
     * monitor RX == client TX
     */
    pd_io_direction_init(&client_links[cid].rx,
     ...
     */
    pd_io_direction_init(&client_links[cid].tx,
...
{
    return cid < PD_IO_CLIENT_COUNT && cid < PC_CHILD_PER_MONITOR_MAX_NUM &&
...
static int monitor_send_pong(uint32_t sender_cid)
{
    ...
    int err = pd_io_direction_send(&client_links[sender_cid].tx,
    ...
    if (err != PD_IO_QUEUE_OK) {
        sddf_printf(PROGNAME "failed to send pong to client %u: %d\n", sender_cid, err);
        return err;
    }
                                    ...
                                    uint32_t payload_len)
{
    ...
    for (uint32_t target_cid = 0; target_cid < PD_IO_CLIENT_COUNT; target_cid++) {
        ...
        if (!monitor_pd_can_receive(target_cid)) {
            sddf_printf(PROGNAME "not forwarding client %u message to passive client %u\n",
                        sender_cid,
                        target_cid);
            continue;
        }
         ...
         */
        int err = pd_io_direction_send(&client_links[target_cid].tx,
                                       (uint8_t)sender_cid,
                                       requested_targets,
                                       payload,
                                       payload_len);
        ...
        if (err != PD_IO_QUEUE_OK) {
            sddf_printf(PROGNAME "failed to forward from client %u to client %u: %d\n",
                        sender_cid,
                        target_cid,
                        err);
            continue;
        ...
        sddf_printf(PROGNAME "forwarded %u bytes from client %u to client %u\n",
                    payload_len,
                    sender_cid,
                    target_cid);
    }
...
void monitor_handle_client_payload(uint32_t sender_cid)
{
    ...
    if (sender_cid >= PD_IO_CLIENT_COUNT) {
        sddf_printf(PROGNAME "invalid sender client ID: %u\n", sender_cid);
        return;
    }

    for (;;) {
        int err = pd_io_direction_receive(&client_links[sender_cid].rx,
                                          &header,
                                          payload,
                                          sizeof(payload),
                                          &payload_len);
        ...
        if (header.source != (uint8_t)sender_cid) {
            sddf_printf(PROGNAME "client %u supplied mismatched source ID %u; "
        ...
        monitor_forward_payload(sender_cid, &header, payload, payload_len);
    }
}
