...
static inline bool service_planner_can_satisfy_request(const protocon_svc_req_t *req,
                                                       const uint32_t avail_service_per_type[])
{
    ...
    for (int type = 0; type < SVC_TYPE_MAX_NUM; ++type) {
        if (req->service_count_per_type[type] > avail_service_per_type[type]) {
            return false;
        }
    }
...
static inline void service_planner_deploy_plan_init(deploy_plan_t *plan, const pc_state_t *state)
{
    assert(plan->req);
    ...
    for (uint32_t i = 0; i < req->service_count; ++i) {
        const protocon_svc_type_t type = req->service_entries[i]->type;

        const uint32_t curr_service_index = --cursor_service_num[type];
                                     ...
                                     const pc_state_t *protocon_states)
{
    ...
    for (int pc_id = 0; pc_id < PC_CHILD_PER_MONITOR_MAX_NUM; ++pc_id) {
    ...
    if (plan->pc_id >= PC_CHILD_PER_MONITOR_MAX_NUM) {
        TSLDR_DBG_PRINT(PROGNAME "No PC has sufficient services for the payload\n");
        return;
    }
    plan->req = req;
