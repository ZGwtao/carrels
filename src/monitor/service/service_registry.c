...
static inline void service_registry_register_protocon_services(const protocon_svcdb_t *services,
                                                               uint32_t pc_id,
                                                               pc_state_t *protocon_states)
{
    ...
    for (uint32_t i = 0; i < services->svc_num; ++i) {
        ...
        const uint32_t service_index = state->resource_quota.avail_service_per_type[type];

        if (service_index >= SVC_PER_TYPE_MAX_NUM) {
            TSLDR_DBG_PRINT(PROGNAME "Too many services of type %u for PC %zu\n",
                            (unsigned int)type,
                            pc_id);
            continue;
        }
...
static inline void service_registry_validate_pc_count(size_t pc_count)
{
