...
bool fs_initialised;
#ifdef FS_MULTIPLEXED
static bool fs_client_initialised[FS_MULTIPLEXER_MAX_CLIENTS];
static uint64_t fs_initialised_clients;
#endif
...
void handle_initialise(void) {
...
#ifdef FS_MULTIPLEXED
    // @gt ?? the arg is generated by multiplexer so is trusted?
    assert(args->client_id < fs_num_clients);
    if (fs_client_initialised[args->client_id]) {
        // double initialisation causes failure
    ...
    if (fs_initialised) {
        ...
        fs_client_initialised[args->client_id] = true;
        fs_initialised_clients++;
...
void handle_deinitialise(void) {
    ...
    assert(fs_initialised_clients > 0);
    if (fs_initialised_clients > 1) {
        fs_client_initialised[args->client_id] = false;
        fs_initialised_clients--;
        args->status = FS_STATUS_SUCCESS;
        return;
    }
